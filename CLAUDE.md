...
The primary client artifact is the interactive HTML Equity Note defined by the approved resource templates. The same file also carries a **slide deck** view — a landscape, presentation-shaped export of the same evidence, reached from the report's **Export slides** control. It lives inside the report rather than being a second artifact, because the exported HTML is the retained research record and a separate deck could drift from it. It prints through the same browser path, with a landscape page box applied only for the duration of that print, and it carries its own disclosure since a deck is shown without the report attached. Its built-in **Print / save PDF** control is the supported PDF path so a saved PDF preserves the same approved layout. The obsolete standalone ReportLab layout must never be presented as the client report.
...
Present it horizontally, one column per criterion, directly under the masthead — the first thing a reader sees after the firm name and rating, ahead of the topline metrics strip. This is the report's most-scanned reasoning, not a buried detail. Each column carries a small info control that explains, in plain language for a non-expert reader, what that criterion measures — separate from the criterion's own detail line, which states this security's actual reading against it. On screen the explanation shows on hover or click/focus; print cannot hover, so the printed report shows the same explanation as a small static caption instead of dropping it.

The five criteria (policy version 2.1, defined in `core/conviction_checklist.py`) are trend (price above both the 50-day and 200-day averages), momentum (MACD above its signal and RSI(14) between 40 and 75), relative strength (return over the lookback beats SPY), quality (return on equity above 15%), and revisions (consensus next-fiscal-year EPS estimate higher than 90 days ago).

Every criterion must actually discriminate. Version 2.0 removed two that did not: "street conviction" (analyst mean target above price) passed 90% of the time across 50 large caps and correlated *negatively* with trend and momentum, so it quietly rewarded stocks that had fallen; and trailing "growth" was the least computable criterion and looks backward, where estimate revisions measure the same question in the direction that moves prices. Trailing growth is still reported in the fundamental section — it is simply no longer a checkbox. Before adding or changing a criterion, measure its pass rate and its correlation against the others on a real sample; a box ticked for nine names in ten cannot separate them.

Every criterion must also be *computable* for the securities the app is actually pointed at. Judge direction rather than magnitude where magnitude is undefined — Revisions reads the direction of the estimate, so a loss-maker whose forecast narrows still confirms it, where an earlier rule requiring a positive base excluded that whole class. Fall back through shorter lookbacks and derived figures before reporting nothing, and state which window or source was used. Where a measure genuinely does not apply — company earnings for a fund — say "not applicable", never "not available", which reads as a retrieval failure a retry might fix.

A criterion that cannot be evaluated from available evidence — a security too new to have a 200-day average, no return-on-equity figure, no consensus estimate at any lookback — is reported as not-confirmed with the reason stated, never guessed, and never silently counted toward or against the score. Changing a threshold, or adding or removing a criterion, is a policy change: bump the version and update the policy file's documentation in the same change — and `web/methodology.html`, the reader-facing explanation of the five, which a test pins to the current policy version and criteria.
